...
import { listWikiPages } from "../src/lib/wiki/blob-store";
...
async function checkExtractionStatus() {
  ...
  for (const chem of chemicals.rows) {
    console.log(
      `  - ${chem.name} (${chem.cas_number}) - SDS: ${chem.source_sds_id}`,
    );
  }

  // Check wiki pages from Blob
  const wikiSlugs = await listWikiPages("chemical/");

  console.log(`\nChemical Wiki Pages (${wikiSlugs.length}):`);
  ...
  console.log(`Chemical Wiki Pages: ${wikiSlugs.length}`);

  const completed = sdsDocs.rows.every(
    (doc: any) => doc.status === "ready" || doc.status === "needs_review",
  );

  if (completed && chemicals.rows.length === 5 && wikiSlugs.length >= 5) {
